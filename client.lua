...
Citizen.CreateThread(function()
	...
	AddTextComponentString("~r~Plasma~b~Game")
    ...
    while true do
        ...
        for k,v in ipairs(PaintBallShop) do
            if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 15.0)then
                DrawMarker(21, v.x, v.y, v.z , 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0, 165, 0, 0, 0,0)
                if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 1.0)then
                    if (incircle == false) then
						if v.typeM == "create" then
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour créer une partie.")
						else
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour rejoindre une partie.")
						end
                    end
                    incircle = true
    ...
    while true do
        ...
        -- for k,v in ipairs(PaintBallShop) do
            if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 5.0)then
                DrawMarker(21, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z , 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0,165, 0, 0, 0,0)
                if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 1.0)then
                    if (incircleRed == false) then
						-- if v.typeM == "create" then
							DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour quitter le jeu.")
						-- else
							-- DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour rejoindre une partie.")
						-- end
                    end
    ...
    while true do
        ...
        -- for k,v in ipairs(PaintBallShop) do
            if(Vdist(pos.x, pos.y, pos.z, mapBlueOutCoords.x, mapBlueOutCoords.y, mapBlueOutCoords.z) < 5.0)then
                DrawMarker(21, mapBlueOutCoords.x, mapBlueOutCoords.y, mapBlueOutCoords.z , 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0,165, 0, 0, 0,0)
...
RegisterNUICallback('validate', function(data, cb)
	...
	if reason == "" then
		...
		notification("Partie créé ! sera supprimé dans 3 minutes s’il n’est pas lancé","succès")
...
mainMenu = UIMenu.New("Plasma Game", "~b~Partie")
...
function openMenuMain(data)
	...
	for k,v in pairs(allSession) do
		if v.CurStat == "WaitingPeople" then
			...
			newitem[k] = _menuPool:AddSubMenu(mainMenu, "Rejoindre '"..tostring(v.creator).." ' partie")
			
			EquipeBleu = v.EquipA
			EquipeRouge = v.EquipB
			
			blueitem[k] = UIMenuItem.New("Equipe Bleu ".. tostring(#EquipeBleu) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(blueitem[k])
			
			reditem[k] = UIMenuItem.New("Equipe Rouge ".. tostring(#EquipeRouge) .."/"..tostring(v.nbpersequip), "")
			newitem[k]:AddItem(reditem[k])
			
			if GetPlayerFromServerId(k) ~= -1 then
				if GetPlayerPed(GetPlayerFromServerId(k)) == PlayerPedId() then
					launchitem[k] = UIMenuItem.New("Lancé le jeu ", "Seul le créateur de la partie peut lancer le jeu")
...
function startGame(color,idx,manche)
	...
	Citizen.CreateThread(function()
		function Initialize(scaleform)
			...
			PushScaleformMovieFunctionParameterString("~g~Manche : "..tostring(manche))
			PushScaleformMovieFunctionParameterString("Ne tirez pas sur vos alliés !")
...
function endTheGame(winner)
	...
	if winner == "tie" then
		...
		if winner == currentTeam then
			...
			message = "~r~Défaite !"
			message1 = "Dommage tu n'étais pas loin !"
		end
	end
...
function leaveTheGame(winner)
	...
	-- print("endTheGame winner : "..tostring(winner).." my team : "..tostring(currentTeam))
	local message = "Trouillard !"
	local message1 = "Fuir n’est pas une solution"
	isCurrentlyOutInGame = false
	-- if winner == "tie" then
		-- message = "~y~Tie"
