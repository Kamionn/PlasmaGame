...
AddEventHandler('PaintBall:JoinBlue', function(idx)
	local player = source
	print("joueur : "..tostring(player).." demande de rejoindre: "..tostring(idx))
	...
	for k,v in pairs(currentPaintBallSession[idx].EquipB) do
		...
		if v == player then
			...
			print("retirer le joueur d’une autre équipe")
		end
	end
	...
	if not alreadyIn then
		if #nbPlayer == nil then
			...
			if #nbPlayer < tonumber(currentPaintBallSession[idx].nbpersequip) then
				...
				TriggerClientEvent("PaintBall:Notif",player,"Vous avez rejoint l’équipe bleu","info")
			else
				TriggerClientEvent("PaintBall:Notif",player,"L'équipe bleu est complète","error")
			end
		end
	else
		TriggerClientEvent("PaintBall:Notif",player,"Tu es déjà dans cette équipe","error")
	end
end)
	...
	if not alreadyIn then
		if #nbPlayer == nil then
			...
			if #nbPlayer < tonumber(currentPaintBallSession[idx].nbpersequip) then
				...
				TriggerClientEvent("PaintBall:Notif",player,"YVous avez rejoint l’équipe rouge","info")
			else
				TriggerClientEvent("PaintBall:Notif",player,"L'équipe rouge est complète","error")
			end
		end
	else
		TriggerClientEvent("PaintBall:Notif",player,"Tu es déjà dans cette équipe","error")
...
AddEventHandler('PaintBall:addPoint', function(color,idx)
	local player = source
	print("joueur : "..tostring(player).." est mort en jeu : "..tostring(idx).." couleur : "..tostring(color))
